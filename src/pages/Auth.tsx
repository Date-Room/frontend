...
/**
 * Magic-link sign-in. The emailed link returns to `/auth/callback`, where
 * supabase-js exchanges the session and routes the user onward (see
 * AuthCallback.tsx). Requires `${origin}/auth/callback` in the Supabase
 * project's Auth → Redirect URLs allowlist.
 */
export default function Auth() {
  ...
  async function handleMagicLink(e: React.FormEvent) {
    ...
    if (!supabaseConfigured()) {
      toast.error("Sign-in isn't configured yet (missing Supabase keys).");
      return;
    }
    setLoading(true);
    try {
          ...
          {sent ? (
            <div className="space-y-5 text-center">
              ...
              <div className="space-y-1.5">
                ...
                <p className="mx-auto max-w-md text-sm font-light leading-relaxed text-muted-foreground">
                  We sent a sign-in link to <span className="text-cream">{email.trim()}</span>. Tap it to come
                  back. (Open it on this device.)
                </p>
              </div>
              <button type="button" className="auth-mode-switch" onClick={() => setSent(false)}>
            ...
            <>
              <div className="mb-6 space-y-1.5 text-center sm:mb-7">
                ...
                <p className="mx-auto max-w-md text-sm font-light leading-relaxed text-muted-foreground">
                  Virtual date rooms for two. Enter your email and we'll send a sign-in link.
                </p>
              </div>
