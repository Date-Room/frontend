  ...
  return (
    <div className={cn("space-y-2", className)}>
      ...
      >
        ...
        <span className="ml-auto text-body text-cream">{active.nativeLabel}</span>
        <ChevronDown
          className={cn(
      ...
      {open && (
        <div className="overflow-hidden rounded-2xl border border-border bg-card/40">
          {APP_LOCALES.map((loc, i) => (
            ...
            >
              <span className="flex-1 text-body text-cream">{loc.nativeLabel}</span>
              <span className="mr-3 text-label text-muted-foreground">{loc.label}</span>
