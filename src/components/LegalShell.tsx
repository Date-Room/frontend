  ...
  return (
    <PageShell>
      <main className="max-w-6xl mx-auto px-5 sm:px-8 pt-10 sm:pt-12 pb-28 relative z-10 animate-fade-in">
        <Link
          to="/"
          className="inline-flex items-center gap-2 rounded-xl px-3 py-2 text-muted-foreground hover:text-cream hover:bg-white/[0.06] transition-colors mb-8 text-body border border-transparent hover:border-white/[0.08]"
        >
        ...
        <header className="mb-10 lg:mb-12 max-w-3xl">
          <p className="text-label uppercase tracking-[0.35em] text-muted-foreground mb-4 flex items-center gap-3">
            <span
          ...
          <h1 className="font-serif font-semibold text-display md:text-4xl lg:text-[2.75rem] text-cream mb-3 tracking-tight">
            {title}
          </h1>
          <p className="text-body text-muted-foreground leading-relaxed">{intro}</p>
          <p className="mt-4 text-label uppercase tracking-[0.28em] text-muted-foreground/80">
        ...
        <div className="lg:grid lg:grid-cols-[14rem_minmax(0,1fr)] lg:gap-12 xl:grid-cols-[16rem_minmax(0,1fr)] xl:gap-16">
          ...
          <details className="group lg:hidden mb-8 rounded-2xl border border-white/[0.08] bg-white/[0.025] backdrop-blur-sm">
            <summary className="cursor-pointer list-none px-4 py-3 flex items-center justify-between text-label uppercase tracking-[0.3em] text-muted-foreground hover:text-cream transition-colors">
              <span>Contents</span>
          ...
          <aside className="hidden lg:block">
            ...
            >
              <p className="text-label uppercase tracking-[0.3em] text-muted-foreground/80 mb-4">
                Contents
              </p>
                  ...
                  return (
                    <li key={s.id}>
                      <a
                        href={`#${s.id}`}
                        className={[
                          "block pl-4 pr-2 py-1.5 -ml-px border-l text-body leading-snug transition-colors",
                          active
          ...
          {/* Prose column */}
          <article className="max-w-prose text-body text-cream/90 leading-relaxed">
            {sections.map((s, i) => (
              <section
                key={s.id}
                id={s.id}
              ...
              >
                <p className="text-label uppercase tracking-[0.32em] text-muted-foreground/70 mb-2 tabular-nums">
                  {String(i + 1).padStart(2, "0")}
                </p>
                <h2 className="font-serif font-semibold text-cream text-title tracking-tight mb-4">
                  {s.title}
                </h2>
        ...
        <footer className="mt-16 pt-8 border-t border-white/[0.06] flex items-center justify-between text-label uppercase tracking-[0.28em] text-muted-foreground/70 max-w-prose lg:ml-[calc(14rem+3rem)] xl:ml-[calc(16rem+4rem)]">
