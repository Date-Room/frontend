    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[85dvh] overflow-y-auto rounded-3xl border-border bg-card shadow-2xl sm:max-w-lg">
        <DialogHeader className="text-left">
          <DialogTitle className="text-title italic text-cream">{title}</DialogTitle>
          <p className="text-body font-normal text-muted-foreground leading-relaxed pt-1">
            ...
            return (
              ...
              >
                ...
                <span className="min-w-0">
                  <span className="block text-body font-medium text-cream">{row.label}</span>
                  <span className="mt-1 block text-label text-muted-foreground leading-relaxed">{row.description}</span>
                </span>
              </button>
            );
          })}
